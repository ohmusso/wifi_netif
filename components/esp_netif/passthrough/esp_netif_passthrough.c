...
#if CONFIG_ESP_NETIF_PASSTHROUGH
...
static const char *TAG = "esp_netif_passthrough";
...
{
    ESP_LOGI(TAG, "passthrough initialization");
...
{
    ESP_LOGI(TAG, "passthrough deinitialization");
    if (!s_netif_initialized) {
...
static esp_err_t esp_netif_init_configuration(esp_netif_t *esp_netif, const esp_netif_config_t *cfg)
{
    ...
    memcpy(esp_netif->mac, cfg->base->mac, NETIF_MAC_ADDR_LEN);
...
{
    ESP_LOGI(TAG, "Transmitting data: ptr:%p, size:%d", data, len);
...
size_t xCurIndex;
MyPacket_t xPaketBuf[5];

void esp_netif_netstack_buf_free(void *pbuf)
{
    ESP_LOGI(TAG, "buf_free");
    if( pbuf != (void*)NULL){
        pbuf = (void*)NULL;
    }
    xCurIndex = 0U;
...
{
    ESP_LOGI(TAG, "buf_ref");
    if( pbuf == (void*)NULL){
        xCurIndex++;
        xCurIndex = xCurIndex % 5;
